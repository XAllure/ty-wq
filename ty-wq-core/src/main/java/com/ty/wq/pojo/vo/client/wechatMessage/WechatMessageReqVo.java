...
public class WechatMessageReqVo extends BaseReqVo {
    ...
    /** 微信发送者 */
    @NotBlank(message = "微信发送者不能为空", groups = Chat.class)
    private String wxIdFrom;

    /** 微信接收者 如果发往群的消息,这个值就是群的wxid 如果是别人私聊给自己的消息,这里就是自己的微信号 */
    @NotBlank(message = "微信接收者不能为空", groups = Chat.class)
    private String wxIdTo;

    /** 消息内容 存json数据 */
    @NotBlank(message = "消息内容不能为空", groups = Chat.class)
